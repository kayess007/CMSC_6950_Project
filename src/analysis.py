...
#     g = df.set_index(date_col)[temp_col].groupby(pd.Grouper(freq="M"))
#     out = pd.DataFrame({
#         "mean": g.mean(),
#         "min": g.min(),
#         "max": g.max(),
...
def monthly_summary(df: pd.DataFrame, date_col: str, temp_col: str) -> pd.DataFrame:
    # >>> CHANGED: old freq="M"  → new freq="ME" (month-end) to remove FutureWarning
...
if __name__ == "__main__":
    ...
    # >>> ADDED: ensure datetime conversion here too
    df[date_col] = pd.to_datetime(df[date_col], errors='coerce')
